...
export default function DashboardPage() {
  ...
  const checkWelcomeCoupon = async () => {
    try {
      ...
      const { data: { session: couponSession } } = await supabase.auth.getSession();
      if (!couponSession?.user) return;

      const { data } = await supabase.rpc('get_welcome_coupon', { p_user_id: couponSession.user.id });
      if (data?.found) {
        setWelcomeCoupon({
          code: data.code,
