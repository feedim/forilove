...
function processTemplateHtml(rawHtml: string, htmlData: Record<string, any>): { sanitizedHtml: string; scripts: string[] } {
  ...
  // Rewrite CSS: scope :root/html/body → .html-template-render + strip scroll-blocking rules
  sanitizedHtml = sanitizedHtml.replace(
    /(<style[^>]*>)([\s\S]*?)(<\/style>)/gi,
    (_, open, css, close) => {
      let fixed = css;
      fixed = fixed.replace(/(html|body)\s*\{([^}]*)}/gi, (_m: string, sel: string, rules: string) => {
        const cleaned = rules
          .replace(/overflow\s*:\s*hidden\s*;?/gi, '')
          .replace(/height\s*:\s*100%\s*;?/gi, '');
        return `${sel}{${cleaned}}`;
      });
      // Scope CSS variables and styles to container instead of document root
      fixed = fixed.replace(/:root(?![a-zA-Z0-9_-])/g, '.html-template-render');
      fixed = fixed.replace(/(?<![.\w#-])html(?![.\w#-])/g, '.html-template-render');
      // body.className stays untouched so script-driven class toggles work on real <body>
      fixed = fixed.replace(/(?<![.\w#-])body(?![.\w#-])/g, '.html-template-render');
...
export function HTMLTemplateRender({ project, musicUrl }: { project: any; musicUrl?: string }) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'lab(49.5493% 79.8381 2.31768)', padding: musicUrl ? '28px 20px 144px' : '28px 20px 72px', gap: 14 }}>
        <span style={{ fontSize: 15, fontWeight: 600, color: '#fff', letterSpacing: '0.01em' }}>Bu şablonu beğendin mi?</span>
        <a href={`https://www.forilove.com/editor/${project.template_id}`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#fff', color: '#e30076', fontSize: 15, fontWeight: 700, padding: '12px 36px', borderRadius: 100, border: 'none', cursor: 'pointer', textDecoration: 'none', letterSpacing: '0.02em', boxShadow: '0 2px 8px rgba(0,0,0,0.12)' }}>
          Sende Oluştur &rarr;
        </a>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', maxWidth: 220 }}>
          <span style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.25)' }} />
          <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.55)' }}>veya</span>
          <span style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.25)' }} />
        </div>
        <a href="https://www.forilove.com/templates" target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, fontWeight: 600, color: '#fff', textDecoration: 'underline', textUnderlineOffset: '3px', textDecorationColor: 'rgba(255,255,255,0.4)' }}>
          Şablonlara Göz At
        </a>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
          <Heart style={{ width: 10, height: 10, fill: 'rgba(255,255,255,0.5)', color: 'rgba(255,255,255,0.5)' }} />
          <span style={{ fontSize: 11, fontWeight: 500, color: 'rgba(255,255,255,0.5)' }}>forilove ile yapıldı</span>
        </div>
      </div>
